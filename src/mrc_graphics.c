...
BITMAP_565 *createBitmap565(int width, int height)
{
  ...
  bmp->bitmap = (uint16 *)my_malloc(width * height * 2);
...
void bitmap565Rotate(BITMAP_565 *b, int type, int flip)
{
  ...
  case 1: // 旋转90度
    newWidth = b->height;
    newHeight = b->width;
    newBitmap = (uint16 *)my_malloc(newWidth * newHeight * sizeof(uint16));
...
BITMAP_565 *createBitmapFromBitmap(BITMAP_565 *bmp, int32 width, int32 height)
{
  ...
  if (bmp->color_bit == 16)
  {
    new_bmp->bitmap = (uint16 *)my_malloc(width * height * sizeof(uint16));
    new_bmp->buflen = width * height * sizeof(uint16);
  }
  else if (bmp->color_bit == 32)
  {
    new_bmp->bitmap = (uint16 *)my_malloc(width * height * sizeof(uint32));
