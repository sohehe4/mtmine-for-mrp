...
int32 _drawText(char* chr, int16 x, int16 y,  uint8 r, uint8 g, uint8 b, E_CHR_SET flag, uint16 font)
{
	if (!chr ||y < 0)
	{
		return MR_FAILED;
	...
	if(useSysFontInfo)
		{
		mrc_drawText(chr,x,y, r, g, b,flag,font);
		return MR_SUCCESS;
		}
	//存在游戏自带字库，使用自带字库
	{
...
int32 _textWidth(char *pcText,E_CHR_SET flag)
{
    int32 tw=0,th=0;
    
    if(pcText)
    {
        _textWidthHeight(pcText,&tw,&th,flag,MR_FONT_MEDIUM);
    }
    return tw;
}
